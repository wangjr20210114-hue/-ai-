...
- 每个会话采用 FIFO。正在生成时，后续消息只显示在输入框上方的队列抽屉中，不提前写入聊天记录；最多等待 5 条。
- 排队消息可编辑、删除，也可提升为下一条并停止当前回答。只有真正开始执行时，它才作为用户消息进入聊天记录。
- 用户消息与对应回答共享同一个 `client_message_id`。切换会话、恢复连接和合并缓存时必须按此字段配对，不能按行号猜测。
- 语音输入属于客户端 Adapter：Web 使用浏览器语音服务，Android、HarmonyOS 与 iOS 可替换为系统或官方语音 SDK；最终文字仍进入同一个输入框并通过 `/chat` 发送，后端业务无需改变。
