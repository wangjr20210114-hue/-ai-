...
export default function TravelPlanCard({ plan, startTs = 0, parsedSchedules, isSaved = false, onSaved, onDeleted }: Props) {
  ...
  const { updateWorkspace } = useMapsController(conversationId);
  ...
  const handleSave = async () => {
    ...
    try {
      if (saved) {
        const savedPlan = await updateWorkspace('save_travel_plan', { plan });
        const updated = savedPlan.travel_plan || plan;
        dispatch({ type: 'UPDATE_PLAN', payload: updated });
        if (scheduleId) {
          const response = await updateWorkspace('direct_calendar_changes', {
            changes: [{
        ...
        const scheduleResponse = changes.length
          ? await updateWorkspace('direct_calendar_changes', { changes })
          : null;
        if (scheduleResponse) {
  ...
  const handleDelete = async () => {
    if (!saved) return;
    try {
      await updateWorkspace('delete_travel_plan', { plan_id: plan.id });
      if (scheduleId) {
        const response = await updateWorkspace('direct_calendar_changes', {
