...
export default function RouteMap({ departure, destination }: Props) {
  ...
  const { planVerifiedRoute, searchVerifiedPlaces } = useMapsController(
    conversationId,
  );
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      ...
      try {
        ...
        setRouteData(await planVerifiedRoute([origins[0], destinations[0]]));
      } catch {
    ...
    fetchRoute();
  }, [departure, destination, planVerifiedRoute, searchVerifiedPlaces, t]);
