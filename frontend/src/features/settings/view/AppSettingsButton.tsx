...
} from '../controller/useSettingsController';
...
export default function AppSettingsButton() {
  ...
  const {
    getReadingSettings,
    intelligence,
    proactive: runProactive,
    providerUsage: fetchProviderUsage,
    resetApplication,
    updateReadingSettings,
  } = useSettingsController(conversationId);
  ...
  const setPreferences = async (changes: Record<string, unknown>) => {
    setBusy('proactive');
    try {
      const next = await runProactive('update_preferences', {
  ...
  const saveSearch = async (changes: Partial<NonNullable<MakersIntelligenceState['search_preferences']>>) => {
    ...
    setBusy('search');
    try {
      const next = await intelligence('update_search_preferences', {
  ...
  const saveMap = async (changes: Partial<NonNullable<MakersIntelligenceState['map_preferences']>>) => {
    ...
    setBusy('maps');
    try {
      const next = await intelligence('update_map_preferences', {
        preferences: nextPreferences,
      });
  ...
  const clearAllData = async () => {
    ...
    setResetError('');
    try {
      await resetApplication(resetConfirmation);
        ...
        {settingsLevel === 'advanced' && skillEnabled('workflow_action') && <section className="app-settings-section">
          ...
          {preferences && <div className="app-settings-grid">
            ...
            <Button size="small" variant="outline" loading={busy === 'scan'} onClick={() => {
              setBusy('scan');
              void runProactive('refresh').then((next) => {
                dispatch({ type: 'HYDRATE_PROACTIVE', payload: next });
