...
const PUBLIC_REQUEST_ERROR = /(?:模型服务暂时未能处理本轮上下文|模型服務暫時無法處理本輪內容|这次没有生成成功|這次未能成功生成|本次生成失败)/i;
const PUBLIC_SERVICE_ERROR = /(?:模型服务配置异常|消息服务暂时异常|模型服務設定異常|訊息服務暫時異常|模型服务暂时不可用|消息服务暂时不可用|模型服務暫時無法使用|訊息服務暫時無法使用)/i;
