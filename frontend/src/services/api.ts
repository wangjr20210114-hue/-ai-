import type { ChatMessage, ConversationSummary, TravelPlan, CityInfo, ScheduleItem, MeetingResult, StoredFileInfo, TravelCollected, MakersMapPlace, MakersRoutePlan, WorkspaceAction } from '../types';
...
function normalizeConversation(item: Record<string, unknown>): ConversationSummary {
  ...
  const updatedAt = Number(item.lastMessageAt || item.updatedAt || item.updated_at || createdAt);
  return {
    id,
    title: String(metadata.title || item.title || '新对话'),
    createdAt: createdAt < 10_000_000_000 ? createdAt * 1000 : createdAt,
    updatedAt: updatedAt < 10_000_000_000 ? updatedAt * 1000 : updatedAt,
    messageCount: Number(item.messageCount || item.message_count || 0),
  };
}

export async function listConversations(): Promise<ConversationSummary[]> {
  const res = await authorizedFetch(isEdgeOne ? '/conversations' : `${BASE}/conversations`);
...
export async function createNewConversation(): Promise<ConversationSummary> {
  ...
  const data = await res.json() as { conversation: Record<string, unknown> };
...
export async function saveConversationMessage(conversationId: string, message: ChatMessage): Promise<void> {
  const res = await authorizedFetch(
    isEdgeOne ? '/conversations' : `${BASE}/conversations/${encodeURIComponent(conversationId)}/messages`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(isEdgeOne
        ? {
        ...
        : { id: message.id, role: message.role, content: message.content, ts: message.ts, metadata: message }),
    },
  );
