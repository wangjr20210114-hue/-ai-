...
import type { ChatMessage, TravelPlan, WSMessage, SkillInfo, MeetingResult, ScheduleItem, WorkspaceAction } from '../../types';
import type { ChatClient } from '../../services/chatClient';
import { cancelPendingAction, confirmPendingAction, saveConversationMessage, waitForPendingAction, workspaceOperation } from '../../services/api';
...
export default function MessageBubble({ message, client }: Props) {
  ...
  const [workspaceActions, setWorkspaceActions] = useState<WorkspaceAction[]>(message.workspaceActions || []);
  ...
  useEffect(() => {
    setWorkspaceActions(message.workspaceActions || []);
  ...
  const replaceWorkspaceAction = (next: WorkspaceAction) => {
    setWorkspaceActions((items) => items.map((item) => item.id === next.id ? next : item));
  };
                  ...
                  <div key={action.id} className="workspace-confirm-card">
                    ...
                    {action.status === 'awaiting_confirmation' ? (
                      ...
                      <div className={`workspace-action-status status-${action.status}`}>
                        {action.status === 'succeeded' ? '✓ 已完成' : action.status === 'cancelled' ? '已取消' : action.status === 'failed' ? `失败：${action.error || '执行失败'}` : '处理中…'}
                      </div>
                    )}
