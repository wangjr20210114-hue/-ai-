...
function ClarificationCard({
  ...
  return <div className="clarification-card">
    ...
    {isSubmitted ? <div className="clarification-complete">{t('filledInput')}</div> : activeField && (() => {
    ...
    {!isSubmitted && <div className="clarification-actions">
      {fields.length > 1 && <Button size="small" variant="outline" disabled={activeIndex === 0 || submitting || generationActive} onClick={() => setActiveIndex((current) => Math.max(0, current - 1))}>{t('previousStep')}</Button>}
      {!lastStep ? <Button size="small" theme="primary" disabled={!currentComplete || submitting || generationActive} onClick={advance}>{t('nextStep')}</Button> : <Button size="small" theme="primary" loading={submitting} disabled={!complete || generationActive} onClick={() => { void submit(); }}>{t('completeAndContinue')}</Button>}
    </div>}
  </div>;
}
