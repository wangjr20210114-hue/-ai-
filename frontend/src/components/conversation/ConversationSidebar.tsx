...
export default function ConversationSidebar({ client, open, onClose }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const currentPending = conversations.find((item) => item.id === conversationId && item.pending);
      const withCurrent = stored.some((item) => item.id === conversationId)
        ? stored
        : [currentPending || pendingConversation(conversationId), ...stored];
      ...
      <aside className={`conversation-sidebar panel ${open ? 'is-open' : ''}`} aria-label="对话历史">
        <div className="conversation-sidebar-header">
          <div>
            ...
            <div className="conversation-sidebar-subtitle">每个对话拥有独立记忆</div>
          </div>
            ...
            >
              ...
              <span className="conversation-item-content">
                ...
                <span className="conversation-item-meta">
                  {conversation.pending ? '尚未发送消息' : `${conversation.messageCount || ''} ${displayTime(conversation.updatedAt)}`.trim()}
                </span>
              </span>
            </button>
          ))}
        ...
        <div className="conversation-memory-note">
          新对话不会读取其他对话内容
        </div>
      </aside>
    </>
