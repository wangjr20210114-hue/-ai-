...
export default function EdgeOnePlatformPanel() {
  ...
  const { connected, conversationId, schedules, mapPlaces, mapTitle, mapRevision, calendarPulse } = useAppState();
  ...
  return (
    <aside className="my-panel makers-workspace">
      <div className="my-panel-card makers-status-card">
        <div className="section-title" style={{ margin: 0 }}>
          行程工作台
          <Tag size="small" theme={connected ? 'success' : 'warning'} variant="light" style={{ marginLeft: 'auto' }}>
            {connected ? '已连接' : '连接中'}
          </Tag>
        </div>
      </div>
        ...
        <div className="calendar-grid">
          {calendarDays.map((date, index) => {
            ...
            return (
              <button
                key={key}
                className={`calendar-day ${isToday ? 'today' : ''} ${isSelected ? 'selected' : ''} ${isPulse ? 'calendar-day-pulse' : ''}`}
                style={{ background: count ? `rgba(78, 124, 255, ${Math.min(0.12 + count * 0.08, 0.5)})` : undefined }}
                onClick={() => { setSelectedDate(date); setShowRecommendation(false); }}
      ...
      <div className="my-panel-card makers-day-card">
        <div className="section-title">
          {selectedDate.getMonth() + 1}月{selectedDate.getDate()}日安排
          <Tag size="small" variant="light" style={{ marginLeft: 'auto' }}>{selectedItems.length} 项</Tag>
        ...
        {formOpen && (
          <div className="makers-schedule-form">
            ...
            <div className="makers-place-search-row makers-place-autocomplete">
              <input
                value={placeQuery}
                onChange={(event) => { setPlaceQuery(event.target.value); setSelectedPlace(null); }}
