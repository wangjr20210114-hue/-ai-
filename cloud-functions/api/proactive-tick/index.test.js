...
function fakeStore({ duplicate = false } = {}) {
...
test('scheduled bridge forwards one tick to the proactive Agent', async (t) => {
  ...
  const request = new Request('https://preview.example/api/proactive-tick?eo_time=1', {
    method: 'POST',
...
test('duplicate platform delivery is a successful no-op', async (t) => {
  const store = fakeStore({ duplicate: true });
  const fetchMock = t.mock.method(globalThis, 'fetch', async () => {
    throw new Error('must not be called');
  });
  const result = await tickUser(
    new Request('https://preview.example/api/proactive-tick', { method: 'POST' }),
    store,
  );
  assert.equal(result.status, 200);
