...
export async function onRequest(context) {
  ...
  if (request.method === 'POST') {
    ...
    const item = {
      ...
      source_url: String(body.source_url || existing?.source_url || '').slice(0, 1200),
      file_size: Math.max(0, Number(body.file_size || existing?.file_size || 0)),
      part_size: Math.max(0, Number(body.part_size || existing?.part_size || 0)),
