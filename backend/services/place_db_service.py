...
DB_HOST = os.getenv("PLACE_DB_HOST", "")
DB_PORT = int(os.getenv("PLACE_DB_PORT", "5432"))
DB_NAME = os.getenv("PLACE_DB_NAME", "osm")
DB_USER = os.getenv("PLACE_DB_USER", "")
DB_PASSWORD = os.getenv("PLACE_DB_PASSWORD", "")
...
async def get_pool() -> asyncpg.Pool:
    ...
    if _pool is None:
        if not DB_HOST or not DB_USER or not DB_PASSWORD:
            raise RuntimeError("地点数据库未配置；请设置 PLACE_DB_HOST/USER/PASSWORD")
...
async def nearby_places(
    ...
    *,
    ...
    if place_type == "restaurant" or place_type == "food":
        ...
        type_filter = "AND (tags->>'amenity' = $5 OR tags->>'tourism' = $5 OR tags->>'leisure' = $5)"
        params.append(place_type[:80])
