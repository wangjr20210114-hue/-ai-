...
## 推断规则
- "帮我画一只猫" → intent=image
- "我想去杭州旅游" → intent=travel
- "明天下午3点开会" → intent=meeting
- "帮我搜一下最新的AI新闻" → intent=search
- "今天天气怎么样" → intent=search
- "明朝那些事儿有什么公众号" → intent=search
- "知乎上怎么评价XXX" → intent=search
- "XXX是什么" → intent=search
- "帮我查一下XXX的百科" → intent=search
- "我想看attention is all you need这篇论文" → intent=paper
- "这篇英文看不懂" → intent=translation
- "你好" → intent=chat

## 参数提取
- travel: {"destination": "城市名", "days": 天数, "departure": "出发城市"}
- meeting: {"subject": "会议主题", "start_time": "ISO时间", "duration_minutes": 时长}
