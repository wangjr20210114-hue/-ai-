...
| 日程闭环 | 让 Floris 把上一条多站路线写入日程，核对确认卡后确认；打开设置 → 我的日程 | 确认卡逐项列出每个站点及冲突/路程提醒，不把多站压成一条；确认前不落库，确认后 Makers 日程页按时间显示 |
...
| PDF 与论文助读 | 输入框左侧“＋”→上传 PDF；打开“我的阅读”→“打开”，在微信原生阅读器复制一段文字；返回点“助读”，粘贴并选择翻译/总结/问答 | 文件写入 Makers Blob 与阅读库；PDF 用 `wx.openDocument` 打开；助读结果通过 `/reader` 流式输出，最新结果在最上方且只保存一次，历史结果刷新后仍存在 |
