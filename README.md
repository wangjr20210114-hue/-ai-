...
| 响应式布局 | 手机使用原生 `tabBar` + 单列内容流，平板/宽屏才展开提醒与最近会话侧栏；所有功能键避开微信右上角胶囊区 |
| 视觉与排版 | 暖橙/夜空双主题共用一套设计令牌；界面与 Markdown 统一使用微信系统无衬线字体，只有 4 个清晰字号层级；AI 回答平铺、结构化操作独立成面，避免卡片套卡片 |
| 交互动效 | 微信原生 `hover-class`、CSS transition/keyframes 与系统减弱动态效果偏好；按压、消息进入、提醒轮播与移动抽屉采用 100–240ms 一次性短动效，不使用永久漂浮或整页反复弹跳 |
...
| 首屏与本地缓存 | 压缩后的 Floris 头像和明暗背景随包加载；`wx.setStorageSync` 立即恢复会话，并为日程、阅读、提醒、设置提供 stale-while-revalidate 快照；页面重开早于运行入库时只对 Makers `/messages` 做短时只读重查，Makers Conversation/Checkpointer 始终是服务端事实源 |
...
| 能力 | 测试人员操作 | 期待结果 |
| --- | --- | --- |
| 微信登录 | 微信开发者工具清除缓存并重新编译 `miniapp/`；不要在 Console 手工伪造 Token | 首页正常打开；Network 中先完成 `wx.login` 对应的 `/wechat-auth`，响应与日志不暴露 `openid`、`session_key` 或 AppSecret |
| 流式问答与停止 | 输入“最近 AI 有什么新进展”；正文开始输出后点击方形停止按钮，再发送“1+1 等于几” | 正文按块持续出现；微信真机不依赖浏览器 `TextDecoder`，中文或 emoji 跨网络分片仍可正确解码；停止后输入区立即恢复，不自动续写、不自动重试；下一条问题可以正常回答 |
| 会话恢复 | 完成一轮问答后打开历史对话，再回到原对话；随后重新编译小程序 | 已完成消息从 Makers 会话恢复；本地缓存只改善首屏，不生成重复用户消息或重复 AI 回答 |
| 必要信息卡 | 请求一条包含含糊同名地点的路线，在卡片中选择具体地点 | 选择结果静默交给同一个模型回答继续思考，不出现在输入框，也不新增一条用户气泡 |
| Skills 隔离 | 设置 → Skills 广场，先关闭地图或日历能力并提相应请求；再开启后重问 | 关闭时模型自然说明该能力未开启，不误调用其他 Skill；开启后同一请求返回真实地图或日程确认卡；依赖能力由现有 Intelligence Agent 补齐 |
| 地图与有序路线 | 请求“先去早餐店，再去北京站，最后去锦江之星”的路线；点击“在地图中查看” | 地点均经过现有 Maps Skill 核实；原生地图 marker 编号与用户顺序一致；路线请求固定 `optimize=false`，不自行重排站点；普通地点推荐不连线 |
| 日程闭环 | 让 Floris 把上一条多站路线写入日程，核对确认卡后确认；再打开底部“日程”，选择今天或未来日期，直接新增、编辑、删除一条日程并搜索地点 | 确认卡逐项列出每个站点及冲突/路程提醒，不把多站压成一条；直接编辑仍复用 Calendar Skill、Makers 持久化与地点核实，完成后主动提醒同步更新；过去日期只读 |
| 图片与版本链 | 请求文生图，再基于结果修改；左右滑动版本并点击“保存到相册” | 图片继续由现有图片 Action 生成；原生 `swiper` 展示全部版本；首次保存按微信原生流程请求相册权限，授权后写入系统相册 |
| 主动提醒 | 设置 1–2 条“无真实提醒时的短句”，返回首页；再授权位置并触发天气/日程相关提醒 | 没有真实提醒时 Header 轮播已持久化短句；真实提醒或主动工作流出现后完全替换短句；页面打开立即检查，前台每 10 分钟复用 Proactive Agent 检查 |
| PDF、阅读库与论文助读 | 输入框左侧“＋”→上传 PDF；打开底部“阅读”，切换自动整理、新建文件夹、移动文档；点“打开”后在微信原生阅读器复制一段文字，返回点“助读”，粘贴并选择翻译/总结/问答 | 文件、文件夹和助读结果均写入 Makers；PDF 用 `wx.openDocument` 打开；文件夹支持新建、改名、删除和文档移动；助读通过 `/reader` 流式输出，最新结果在最上方且只保存一次 |
| 回答复制 | 完成一轮 Markdown 回答，点击回答右上角复制图标，再粘贴到微信输入框 | 复制内容为可读纯文字，不包含图片或 URL 目标；原回答仍保留 Markdown 与富媒体排版 |
