"""Signed multi-user identity shared by every Makers Agent route."""
...
def require_user(ctx: Any) -> dict[str, Any]:
    ...
    payload = _verify_session(_cookie(ctx, "floris_session"), secret)
    tenant_id = _safe_segment(payload.get("tenant_id"), "default")
    subject_id = _safe_segment(
        payload.get("sub") or payload.get("subject_id"),
        "anonymous",
    )
    auth_type = str(payload.get("auth_type") or "guest")
    if auth_type not in {"guest", "wechat"}:
        raise AuthError("登录身份类型无效")
    membership = str(payload.get("membership") or "")
    if membership not in {"guest", "free", "plus", "pro"}:
        membership = "guest" if auth_type == "guest" else "free"
...
def tenant_storage_prefix(identity_or_user_id: Any) -> str:
    ...
    if isinstance(identity_or_user_id, dict):
