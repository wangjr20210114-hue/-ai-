...
from ._capability_plan import plan_capabilities, tool_names_for_plan
...
rich_search 仅在本轮语义路由开放时可用。需要发现、比较、推荐、网页事实或视觉资料时使用；地点定位、日程地点核实等轻量任务不要尝试调用不可用的工具。搜索结果只是素材和证据，不限制你使用自身知识、措辞、观点或回答结构。不要用网页列表代替综合回答，也不要为了展示工具而罗列素材。
搜索返回的网页、图片、视频等素材由你自由编排：只采用真正有助于当前叙述的项目，把它放在最相关的段落附近；可以交错使用、重排或全部舍弃。不要把素材统一堆在回答末尾。使用工具给出的原始 Markdown URL，前端会在你选定的位置渲染对应图片、视频或网页卡片。
    推荐一个餐馆、景点或其他地点时，必须同时使用 rich_search 和 search_places。仅当用户询问单个地点在哪里、并明确要求在地图显示时，不要调用 rich_search：调用 search_places 核实该地点，再调用 prepare_map_recommendation 传一个 place_id 和 expected_place_count=1。推荐两个及以上地点或路线时必须调用 search_places_batch，把回答中的每个地点作为独立 query 核实，并从每组选择一个最匹配的真实 place_id。只有地点工具返回的真实 place_id 才能交给 prepare_map_recommendation；未验证地点可以在正文中明确说明，但不能进地图。
    prepare_map_recommendation 生成可安全激活的地图 Action；网页会在本轮首次收到后自动更新一次右侧地图，同时保留按钮供用户查看其他内容后再次恢复该组地点。单地点地图显示的 expected_place_count=1；多地点推荐时 expected_place_count 必须等于回答中实际推荐的地点数，place_ids 必须覆盖每组；数量不足时继续核实，不能创建残缺 Action。action_text 要根据上下文自然生成，避免每次使用同一句话。
