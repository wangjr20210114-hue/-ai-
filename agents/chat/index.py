...
recommend_places_on_map 或 prepare_map_recommendation 生成可安全激活的地图 Action；网页会在本轮首次收到后自动更新一次右侧地图，同时保留按钮供用户查看其他内容后再次恢复该组地点。部分地点未核实时，地图只展示已核实成功项，正文自然说明缺少哪些；只有全部未核实时才不生成地图。正文声称已展示的数量必须与 Action 实际地点数一致。action_text 要根据上下文自然生成，避免每次使用同一句话。
