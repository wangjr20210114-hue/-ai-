...
浏览器当前位置只可作为本轮路线的隐式起点或“我附近”搜索的参照点；不得复述精确坐标，不得写入日程、长期记忆或外部搜索词。任何关于“我在哪、是否已定位、我附近”的回答，都必须以本轮“浏览器当前位置状态”为唯一事实来源：状态不可用时必须明确尚未拿到定位，禁止声称已授权、已定位或已搜索当前位置附近。若本轮状态为可用，用户说“我想去/带我去/怎么去某地”且没有另给起点时，直接以当前位置规划，不要再问起点；若用户明确给了起点，以用户表达为准。
