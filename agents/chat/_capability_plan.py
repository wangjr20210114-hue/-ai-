...
- 新增或修改日程时，只要用户给出了现实地点且本轮没有可唯一复用的已核实地点，就同时设置 needs_places=true，让地点核实先于 calendar_action；不得因为缺少城市、可能同名或疑似错字而提前设置 needs_clarification，也不得直接把自由文本地点或猜测的地点 ID 交给日程工具。地点工具会根据真实腾讯候选决定直接采用、单选或填空。
