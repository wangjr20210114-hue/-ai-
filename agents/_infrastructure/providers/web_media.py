...
def _public_host(hostname: str) -> None:
    if not hostname:
        raise ValueError(text("web_media.host_required"))
    for info in socket.getaddrinfo(hostname, None):
        address = ipaddress.ip_address(info[4][0])
        if not address.is_global:
            raise ValueError(text("web_media.private_forbidden"))
...
async def _collect(page_url: str, limit: int) -> list[str]:
    ...
    if parsed.scheme not in {"http", "https"}:
        raise ValueError(text("web_media.scheme_required"))
    ...
    if "text/html" not in content_type:
        raise ValueError(text("web_media.html_required"))
    parser = _ImageParser()
...
async def _collect_media(page_url: str, limit: int) -> list[dict[str, str]]:
    ...
    for match in re.finditer(r'<meta[^>]*(?:property|name)=["\'](?:og:image|twitter:image)["\'][^>]*content=["\']([^"\']+)', html, re.I):
        add(match.group(1), context=text("web_media.hero_context"))
