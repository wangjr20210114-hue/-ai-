...
| 混元视觉 | `HUNYUAN_IMAGE_API_KEY` | 真实公网图片请求在 30 秒内超时 | 当前不适合作为 7 秒富搜索同步审核；搜索仍可使用网页原图，视觉审核超时不阻断正文 |
...
Cloudflare 5016 的含义与许可要求见官方 [Workers AI 错误文档](https://developers.cloudflare.com/workers-ai/platform/errors/) 和 [Llama 3.2 Vision 模型页](https://developers.cloudflare.com/workers-ai/models/llama-3.2-11b-vision-instruct/)。接受第三方模型条款是账号级外部操作，不属于普通代码测试。
