...
4. 搜索通过 Agent 内的 `rich_search` 统一接入 WSA SearchPro，并使用 TokenHub `hy-vision-2.0-instruct` 做网页图片相关性筛选；不保留固定 IP 搜索代理。
...
| 旧多源搜索和固定公网 IP | WSA `rich_search` + HY-Vision 视觉筛选 | `agents/shared/rich_search.py` |
...
原项目的论文库、主动 Collector、备份恢复等属于结构化业务数据或本地设备能力。Makers 的 conversation store 不是关系数据库，不应把这些表强行塞入会话消息。本次处理方式是：

- 本地模式继续由 `backend/` 提供这些能力；
- Makers 模式不启动旧 FastAPI，也不代理到固定服务器；
- Makers 已把地图、真实道路路线、用户级日程、会议 Bridge 和基础生图迁入 Agent/Workspace；
- Makers 前端仍隐藏依赖旧本地 API 的论文、主动运行、记忆、预算、备份和调试面板；
- 后续的图片、文档与阅读资产使用 LangGraph store 保存小型索引和状态、Blob 保存大字节；如果容量或查询需求超出平台 Store，再引入外部数据库，不预先复制 SQLite 架构。

当前迁移事实与下一阶段计划以 [`CURRENT_ARCHITECTURE_AND_REFACTOR_PLAN.md`](CURRENT_ARCHITECTURE_AND_REFACTOR_PLAN.md) 为准。
