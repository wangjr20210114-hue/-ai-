...
当前分支 2026-07-20 的已验证结果：Cloud Functions/平台约束 17 项、前端 Vitest 39 项、Python Agent 88 项、迁移工具 2 项均通过；ESLint、TypeScript/Vite 构建和 `git diff --check` 通过。主包体积警告是已知非阻断优化项。
...
- 自动记忆内容不在网页展示；MEM-01/02 通过跨会话行为验收。清理只能在专用 Preview 使用“主动策略与预算 → 自动记忆控制 → 清除全部自动记忆”，禁止在 Production 清除正式数据。
