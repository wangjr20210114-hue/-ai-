...
当前分支 2026-07-24 的已验证结果：Node Cloud Functions/架构门禁 22 项、前端 Vitest 103 项（19 个测试文件）、Python Agent 171 项；ESLint、TypeScript/Vite 构建和 `git diff --check` 通过。本轮把必要信息卡提升为所有问答场景可用的完整上下文能力，同时保留未决非阻断偏好多方案约束；路线转日程会复用上一轮已核实地点，模型自行假设的时间不再当作用户确认。一次提交自动续答与 280 毫秒主题切换回归继续保留。Provider 脱敏实测见 [`PROVIDER_HEALTH_2026-07-22.md`](PROVIDER_HEALTH_2026-07-22.md)。运行完整 Python 测试前必须激活 `.venv`；系统 Python 缺依赖属于本地环境错误。主包体积警告是已知非阻断优化项。
...
本轮重点人工回归位于测试站 CORE-04、CORE-09、SEARCH-01、SEARCH-05、TRAVEL-04、TRAVEL-05 和 CAL-04：分别检查流式划词复制、所有问答场景的全局结构化澄清、行内来源、追问并行、定位恢复、真实周边路线，以及“先规划路线、再写入日程”的必要时间选择与已核实地点复用。定位使用浏览器 5 分钟位置缓存并监听地图容器尺寸变化；失败时页面必须提供可点击的重试，而不是静默无响应。
