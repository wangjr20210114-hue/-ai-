...
当前分支 2026-07-22 的已验证结果：Cloud Functions/Schedule 适配/平台约束 20 项、前端 Vitest 56 项、Python Agent 123 项、迁移工具 2 项均通过；ESLint、TypeScript/Vite 构建和 `git diff --check` 通过。地图测试同时覆盖“3 个候选仅 2 个核实成功时只展示 2 个”“全部候选均未核实时拒绝生成地图”“同一地图 Action 在切换日历后可再次显式展开”“点击后先显示冻结的核实地点，再持久化激活状态”，以及“地图 SDK 超时后保留地点列表和重试”。前端新增覆盖同一用户轮次 checkpoint/SSE 完全相同回答去重、不同用户轮次重复回答保留，以及远端检查点不含媒体时保留本地渐进式图片。腾讯会议测试覆盖缺失时间保留、卡片内编辑、冲突重新计算、逐条/全选接受冲突、修改时间和取消，以及创建成功后只写入一条日程；企业签名兼容通道已删除，只测试个人官方 MCP。Provider 脱敏实测见 [`PROVIDER_HEALTH_2026-07-22.md`](PROVIDER_HEALTH_2026-07-22.md)。运行 Python 测试前必须先激活上一步创建的 `.venv`；直接使用未安装依赖的系统 Python 会报依赖缺失，这属于本地环境错误而不是代码失败。主包体积警告是已知非阻断优化项。
...
1. 页面显示 66 条当前范围测试用例（包含 Apple 风格短动效与 Reduce Motion 验收）。
