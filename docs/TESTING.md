...
当前分支 2026-07-21 的已验证结果：Cloud Functions/Schedule 适配/平台约束 20 项、前端 Vitest 44 项、Python Agent 112 项、迁移工具 2 项均通过；ESLint、TypeScript/Vite 构建、`edgeone makers build` 和 `git diff --check` 通过。运行 Python 测试前必须先激活上一步创建的 `.venv`；直接使用未安装依赖的系统 Python 会报 `langchain_core` 缺失，这属于本地环境错误而不是代码失败。主包体积警告是已知非阻断优化项。
