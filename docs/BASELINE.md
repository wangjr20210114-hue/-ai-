...
| 全局必要信息收集 | 已实现并测试 | 搜索、路线、写作、翻译、生图、文档、日程、会议共用 `ask_user_clarification`，但仅在缺失字段阻断所有安全有用回答或无法唯一确定副作用对象时生成一张最小结构化卡。非必要偏好和用户尚未决定的事项直接给出 2–3 套带假设与取舍的方案 | `agents/chat/_capability_plan.py`、`agents/chat/_graph.py`、`agents/chat/_ui_tools.py`、`MessageBubble.tsx` |
