...
| 主动日程/天气/路线提醒 | 已实现，平台触发受默认域名保护阻塞 | 打开网页及相关日程变更时刷新；未读风险由模型在空白新对话中自然发起，策略设置位于统一设置；EdgeOne Schedule 触发顶层 `/proactive-tick` Cloud Function，复用 Makers Blob 原子锁后转给 `/proactive` Agent。2026-07-22 两次生产探针证明 Schedule 会按时发出请求，但受保护的默认 `edgeone.cool` 部署域名在进入函数前返回 500（0ms/0MB）；绑定稳定自定义域名后复验 | `cloud-functions/proactive-tick`、`agents/proactive`、`useSSEChat.ts` |
