...
12. 只有信息缺失会阻断所有安全有用的回答，或无法唯一确定真实副作用对象时，语义规划器才把本轮收束为一次最小结构化澄清；搜索、路线、写作、翻译、生图、文档、日程、会议共用同一张卡。非必要偏好和用户尚未决定的事项不再发问卷，而是直接给出 2–3 套写明假设与取舍的方案。必要字段仍优先单选/多选、判断和日期选择器，仅不可枚举内容使用短文本。
...
20. 纯图形操作补齐当前语言的悬浮功能说明和读屏名称；流式回答在用户向上阅读时立即退出自动跟随，只有真正回到底部才恢复，避免底部附近反复吸附造成抖动。每次重新打开网页都会触发一次记忆优先的主动提醒检查，页面内仍保持 10 分钟窗口检查；搜索、地点、论文和图片阶段统一使用短而易懂的状态文案。
...
- Python Agent：167 项通过。
- Node Cloud Functions、平台约束和验收持久化：22 项通过。
- 前端 Vitest：101 项通过，18 个测试文件。
- SQLite 只读迁移工具：2 项通过。
- ESLint：通过。
