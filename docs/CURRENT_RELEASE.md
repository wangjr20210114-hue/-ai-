...
- Preview 提交：`32ccf95bbaefbe9e09256bdd788a266546c11804`
- Preview Deployment：`dpmnthmfw7fx`
- 创建时间：2026-07-21 22:56（Asia/Shanghai）
- 构建结果：成功。
...
| Preview | `dpmnthmfw7fx` | `32ccf95` | 当前 Preview；成功；地点显示、会议多选条件卡和 Cloudflare img2img 真实调用在线通过。 |
...
- Cloudflare Workers AI 的测试 Token 与 Account ID 均只配置到 Preview，生产环境不继承。多模态理解、Flux 英文文生图和 img2img 真实调用均已通过；最终 img2img 日志为 `provider=cloudflare`、`reference_count=1`、`fallback=False`。但 GLM-4.7-Flash 中文翻译预处理在真实 Preview 返回不可用形态，本次以原中文提示词继续生成，虽然有图片结果，颜色、构图和禁字约束的跟随质量不足。因此 Cloudflare 当前定位是“外部主 Provider 失败时仍能出图”的免费降级，不与混元质量等价。SearchPro 图片链路已真实成功过，但视觉 Provider 不可用或候选不相关时会诚实不展示图片，不再声称图片正在生成。混元文生图和图生图已真实通过。
