...
7. 文档上传信号通过 `/proactive` 的 `ingest_signal` 进入同一闭环；独立语义模型从有界短预览在翻译、摘要、行动项或论文结论中选择最有价值的一项。外文 PDF 可在用户没有先说“翻译”时主动建议当前界面语言版本；短预览不写入持久事件。
...
11. 主动停止拥有高于网络恢复的优先级；取消意图在当前浏览器会话持久化，刷新、切换对话和网络恢复均不会重启已停止生成。
12. 缺少必要信息时，语义规划器把本轮收束为唯一一次结构化澄清；搜索、路线、写作、翻译、生图、文档、日程、会议共用同一张卡。有限候选优先单选/多选，是非条件用判断，日期时间用选择器，仅不可枚举内容使用短文本，不再在长回答末尾追加问题。
13. 论文划词与右键翻译写入 Makers Blob 阅读库，并在助读界面按时间追加为可滚动译文记录；自然语言日程删除只删除目标 ID，服务端拒绝未变日程的完全相同重复创建。
14. 前端提供简体中文、繁体中文、English、可爱喵喵语、冷酷喵喵语五种界面语言；语言偏好随聊天请求传给 Agent，论文助读也遵循同一语言指令。
15. 回答图片导出固定使用气泡实测尺寸并去掉视觉边框，避免导出边框与内容错位；日程编辑提示、输入、按钮统一字号。
16. 浏览器定位按钮每次点击都会重新查询权限；定位超时或暂时不可用不会把已授权状态误降为“待询问”。Markdown 来源链接使用浏览器原生新标签行为，点击会先暂停消息列表自动跟随，避免原对话滚动位置跳动。
17. 澄清问题只允许来自当前目标、最近对话、直接相关的安全记忆或当前可核验状态；工具 Schema 不再用具体旅游画像作示例，未决定、否定和临时备选项不会进入长期记忆。
18. 对话皮肤提升到完整中间栏并延伸到输入区，左右工作区不受影响；设置与 Skills 弹窗使用固定加载骨架并移除重复入场动画。设置按语言、搜索、主动服务、阅读整理排序，搜索结果数、候选图片数和图片并行开关仅在实时搜索 Skill 开启时展示。
...
生产自定义域名为 `https://floris.jlutx.com`；任意主机访问同一网址及 `/test-cases/` 都可读取和编辑同一份 Makers Blob 数据。详细启动、部署和恢复步骤见 [`ACCEPTANCE_SITE.md`](ACCEPTANCE_SITE.md)。

## 自动化证据

- Python Agent：164 项通过。
...
- 默认 `edgeone.cool` 地址仍受 3 小时访问保护；长期公开生产入口使用 `https://floris.jlutx.com`。
- DOC/DOCX、扫描 PDF OCR 和页码级引用不在本版完成范围。
- 无法保证所有外部搜索都在 5–10 秒完成；目标是单轮一次 `rich_search`、并行事实/视觉查询、缓存命中时优先快速返回。

## 发布与回滚

- 发布：推送 `main` 后由 EdgeOne 自动构建 Production；必须确认状态成功，再对 `https://floris.jlutx.com` 和 `/test-cases/` 做非破坏性冒烟。
