...
12. 结构化必要信息卡是搜索、写作、翻译、生图、文档、路线、地图、日程、会议和普通问答共用的全局交互能力：独立语义规划器可以要求立即发卡，完整上下文模型在其他场景仍保留该能力，用于识别只在历史对话中才能发现的阻断条件。可选偏好或用户尚未决定但仍可安全回答时禁止发卡，必须直接给出 2–3 套写明假设与取舍的方案；整个判断不匹配具体偏好关键词。必要字段优先单选/多选、判断和日期选择器，仅不可枚举内容使用短文本；提交后答案直接作为当前对话补充信息自动续答。
...
18. 对话皮肤提升到完整中间栏并延伸到输入区，左右工作区不受影响；设置与 Skills 弹窗直接展示稳定内容并移除重复入场动画。设置按语言、搜索、主动服务、阅读整理排序，搜索结果数、候选图片数和图片并行开关仅在实时搜索 Skill 开启时展示。Skills 广场的“实时搜索”是唯一搜索开关；输入区无效的单轮勾选框和相关消息参数已经删除。
19. 设置与 Skills 广场改为立即展示、后台轻量刷新。打开设置不再触发主动模型检查，也不再读取整份论文索引；阅读整理只读独立 settings Blob。Skills 状态与腾讯会议配置由一次 Intelligence Store 读取返回，不再为了一个 Provider 标志加载完整系统健康、工作区和主动服务状态。
20. 纯图形操作补齐当前语言的悬浮功能说明和读屏名称；流式回答在用户向上阅读时立即退出自动跟随，只有真正回到底部才恢复，避免底部附近反复吸附造成抖动。每次重新打开网页都会触发一次记忆优先的主动提醒检查，页面内仍保持 10 分钟窗口检查；搜索、地点、论文和图片阶段统一使用短而易懂的状态文案。深浅主题整页过渡由 1.1 秒缩短为 280 毫秒，并降低模糊强度。
21. 路线转日程闭环会复用上一轮已由地点/路线 Provider 核实的唯一地点，不要求模型跨轮复制脆弱的 Provider ID。真实副作用参数只接受用户明确表达、结构化选择或当前可核验状态；模型在旧回答中自行建议或假设的出发时间不算用户确认，缺少时必须先显示日期时间卡，不能虚构时间或退回“工具不可用”。
...
- Python Agent：171 项通过。
- Node Cloud Functions、平台约束和验收持久化：22 项通过。
- 前端 Vitest：103 项通过，19 个测试文件。
- SQLite 只读迁移工具：2 项通过。
- ESLint：通过。
