...
12. 只有信息缺失会阻断所有安全有用的回答，或无法唯一确定真实副作用对象时，语义规划器才开放一次最小结构化澄清；规划器判定为非阻断或尚未决定时，主模型不再取得澄清工具，必须直接给出 2–3 套写明假设与取舍的方案。该门禁不匹配任何具体偏好关键词。必要字段仍优先单选/多选、判断和日期选择器，仅不可枚举内容使用短文本；用户提交卡片后，答案直接作为当前对话补充信息自动续答，不再回填输入框等待二次发送。
