# 主动式 Agent 改造实现状态（旧 FastAPI 本地链）

> **归档说明（2026-07-15）**：本文描述 `backend/` 的本地 FastAPI + SQLite 实现，不代表 EdgeOne Makers 生产能力。生产现状、架构评价和迁移计划请以 [`CURRENT_ARCHITECTURE_AND_REFACTOR_PLAN.md`](CURRENT_ARCHITECTURE_AND_REFACTOR_PLAN.md) 为准。

> 更新日期：2026-07-12
> 实现分支：`agent/proactive-runtime-refactor-v2`
...
- Chat 与搜索 Skill 共用结构化图文协议：事实引用 `source-*`，图片引用 `media-*`，模型不直接控制远程图片 URL。
- 图片候选在后端进行公网 URL 校验、去重并绑定原始页面；前端按可信元数据解析、显示说明和出处，加载失败时保留可读降级提示。
...
| 搜索可信度 | 有多源检索、安全抓取、稳定 citation/media ID 与可重复排序 | 增加引用覆盖率校验、来源新鲜度评分和冲突证据专用展示 |
